    ...
    if (credentials.secret) {
      ...
      return {
        ...
        encryption: {
          ...
          machineKey: new Uint8Array(
            Buffer.from(credentials.encryption.machineKey, "base64"),
          ),
        },
      };
    }
...
export async function updateProfiles(profiles: unknown[]): Promise<void> {
  ...
  await updateSettings((settings) => {
    ...
    const activeProfileStillExists =
      activeProfileId &&
      validatedProfiles.some((p) => p.id === activeProfileId);
