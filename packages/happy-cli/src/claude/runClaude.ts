...
export async function runClaude(
  ...
  const happyServer = options.happyInject
    ? await startHappyServer(session)
    : null;
  if (happyServer) {
  ...
  const exitCode = await loop({
    ...
    messageQueue,
    api,
    allowedTools: happyServer
      ? happyServer.toolNames.map((toolName) => `mcp__happy__${toolName}`)
      : [],
  ...
  // Stop quota interval
  clearInterval(quotaInterval);
  logger.debug("Stopped quota interval");
